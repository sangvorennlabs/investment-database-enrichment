...
original_json_contact = json.load(open('sample_apollo/contact/contact-original.json'))
original_json_firm = json.load(open('sample_apollo/firm/firm-original.json'))
apollo_enrich_json_contact = json.load(open('sample_apollo/contact/contact-apollo-enriched.json'))
apollo_enrich_json_firm = json.load(open('sample_apollo/firm/firm-apollo-enriched.json'))
...
if input_password in ['rennlabs123***']:
    ...
    with col1:
        if navigation == "Contact":
            ...
            original_json_dict = {key: original_json_contact[idx][key] for key in ['first_name', 'last_name', 'email', 'phone_number', 'city', 'state', 'country', 'linkedin_url', 'facebook_url', 'twitter_url', 'investor_type', 'check_size_max', 'check_size_min', 'investment_stages', 'investment_strategies', 'strategic_sectors', 'geographic_preferences', 'work_history']}
